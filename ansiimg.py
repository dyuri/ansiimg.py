...
if __name__ == '__main__':
    ...
    for y in range(im.size[1]):
        for x in range(im.size[0]):
            ...
            if mode == "bg":
                sys.stdout.write("\033[48;5;%sm%s" % (short, " " * charnum))
            elif mode == "truecolor":
                sys.stdout.write("\033[38;2;%s;%s;%sm%s" % (p[0], p[1], p[2], random_chars(charnum) if sum(p) > 0 else " " * charnum))
            else:
                sys.stdout.write("\033[38;5;%sm%s" % (short, random_chars(charnum) if sum(p) > 0 else " " * charnum))
